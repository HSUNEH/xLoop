...
### 0a. Check for Pipeline Spec

먼저 Pipeline Spec이 있는지 확인한다:
...
**spec.json이 존재하면 (Phase 0 → Phase 1 연결):**
- spec에서 초기 쿼리와 제약 조건을 자동 로드
- `spec.goal.deliverables` → 리서치 주제로 사용
- `spec.domain.target` → 초기 검색 쿼리로 사용
- `spec.domain.constraints` → 검색 필터로 적용
- `spec.pipeline.tools` → 소스 자동 선택

사용자에게 안내:
> Pipeline Spec이 감지되었습니다.
> **주제:** <deliverables>
> **대상:** <target>
> **제약:** <constraints>
>
...
확인 후 → 0c로 이동 (세션은 이미 pipeline 타입으로 존재).

**spec.json이 없으면 → 기존 플로우 (0b)로 진행.**

### 0b. Create or resume a session
